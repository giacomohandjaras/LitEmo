...
%%%%%%%%%%%%%%%%%%%%
%%%% Remove countries with less than 3 authors
%%%%%%%%%%%%%%%%%%%%
countries=covariates.final_authors_country;
countries_tab=tabulate(countries);
countries_count=cell2mat(countries_tab(:,2));
countries_to_remove=countries_tab(countries_count<3,1);
[countries_mask]=SNLP_searchWords(countries, countries_to_remove);

covariates.final_authors(countries_mask==1)=[];
covariates.final_authors_continent(countries_mask==1)=[];
covariates.final_authors_country(countries_mask==1)=[];
covariates.final_authors_gender(countries_mask==1)=[];
covariates.final_authors_name(countries_mask==1)=[];
covariates.final_authors_nobel(countries_mask==1)=[];
covariates.final_authors_prizes(countries_mask==1)=[];
covariates.final_authors_pubyear(countries_mask==1)=[];
covariates.final_authors_translated(countries_mask==1)=[];
covariates.final_authors_count=numel(covariates.final_authors);
data_final(countries_mask==1,:)=[];
data_fixed(countries_mask==1,:)=[];

%%%assegniamo un numero a ciascun paese per fare hot encoding dopo
countries=covariates.final_authors_country;
countries_tab=tabulate(countries);
covariates.final_authors_country_id=zeros(covariates.final_authors_count,1);

for c=1:size(countries_tab,1)
[countries_mask]=SNLP_searchWords(countries, countries_tab{c,1});
covariates.final_authors_country_id(countries_mask==1)=c;
end
...
results_p_coeffs=nan(size(data_final,2),67);
results_t_coeffs=nan(size(data_final,2),67);
results_beta_coeffs=nan(size(data_final,2),67);
...
results_t_perm=nan(permutations,size(data_final,2),67);
...
male_country=covariates.final_authors_country_id(covariates.final_authors_gender==0);
female_country=covariates.final_authors_country_id(covariates.final_authors_gender==1);
...
male_country(isnan(male_data))=[];
female_country(isnan(female_data))=[];
...
all_country=cat(1,male_country,female_country);

X=cat(2,all_gender,all_translated,all_pubyear,all_country);

clear all_gender all_translated all_pubyear all_country male_pubyear female_pubyear male_translated female_translated male_country female_country 


%%%%%One hot encoding for countries using x2fx
...
if(pvalue_perm>1); pvalue_perm=1; end %%%to fix approximation errors
...
results_beta_coeffs_corrected=results_beta_coeffs(h,[2,67]);
results_t_coeffs_corrected=results_t_coeffs(h,[2,67]);
results_t_perm_corrected=results_t_perm(:,h,[2,67]);
results_p_perm_corrected=results_p_perm(h);
results_p_coeffs_corrected=results_p_coeffs(h,[2,67]);
...
if(pvalue_coeff>1); pvalue_coeff=1; end %%%to fix approximation errors
else
[pvalue_coeff,critical_value_at_p]=pareto_right_tail(temp_null,probe_value,0.05);
if(pvalue_coeff>1); pvalue_coeff=1; end %%%to fix approximation errors
end
pvalue_coeff_twotails=pvalue_coeff*2;  %%%%doubling the p-val, since is a two sided test (considering the tail symmetric);
if(pvalue_coeff_twotails>1); pvalue_coeff_twotails=1; end %%%to fix approximation errors
results_p_perm_coeffs_corrected(term,coeff)=pvalue_coeff_twotails;
end
end

clear term coeff pvalue_coeff pvalue_coeff_twotails critical_value_at_p probe_value temp_null
...
y=data_final(:,term);
X=cat(2,covariates.final_authors_gender,covariates.final_authors_translated,tiedrank(covariates.final_authors_pubyear),covariates.final_authors_country_id);
...
%%%%%Residuals for SEX & COUNTRY
new_X_temp=new_X;
new_X_temp(:,[2,5:66,67])=[];
beta_temp = ((new_X_temp'*new_X_temp)^-1)*new_X_temp'*(y); % Coefficients
residual_temp=(y-(new_X_temp*beta_temp))+mean(y);
residual_temp(residual_temp<0)=0;
...
save('ALL_TERMS/all_terms_010_1000perm_v20230714_revision.mat');

clear results_f_perm results_t_perm
save('ALL_TERMS/all_terms_010_1000perm_light_v20230714_revision.mat');
