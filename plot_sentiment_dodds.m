...
load('HEDO/sentiment_analysis_dodds_v20220411.mat');
title_labels={'Happiness','Percentage of words used by authors and present in Dodds''s ratings','Number of Dodds'' words used by authors'};

data_final=nan(numel(frequency),3); %1 for happiness, 2 for the raw frequency, 3 num of used words
for i=1:numel(frequency)
temp=double(frequency{i}); %%%transform occurrences from uint32 to double
data_final(i,2)=sum(temp)./numel(dodds_words{i})*100; %%%evaluate the overall frequency

temp_freq=(temp./numel(dodds_words{i}));
temp_mask=(temp_freq==0);
temp_freq(temp_mask)=[];
temp_freq_rank=tiedrank(temp_freq);
data_final(i,3)=numel(temp_freq_rank)./numel(temp_mask)*100; %%%evaluate how many words in the dictionary were used by the author

temp_var=(dodds_var(temp_mask==0,1).*log10(temp_freq_rank)); %%%multiply the emotional score by the frequency
data_final(i,1)=sum(temp_var)/sum(log10((temp_freq_rank))); %extract the average score for happiness
end


%%%% Plot historical trends of happiness and frequency of words
term=1; plot_ngram_viewer_gender(title_labels(term),'happiness scoring',authors.final_authors_pubyear,data_final(:,term),authors.final_authors_gender)
if (save_derivatives==1)
image_filename = strcat('derivatives/plot_sentiment_dodds_intime_happiness.png');
export_fig(image_filename,'-m6','-nocrop', '-transparent','-silent');
end

pause(5)
term=2; plot_ngram_viewer_gender(title_labels(term),'percentage (%)',authors.final_authors_pubyear,data_final(:,term),authors.final_authors_gender)
if (save_derivatives==1)
image_filename = strcat('derivatives/plot_sentiment_dodds_intime_percentage_authors.png');
export_fig(image_filename,'-m6','-nocrop', '-transparent','-silent');
end
...
if (save_derivatives==1)
image_filename = strcat('derivatives/plot_sentiment_dodds_intime_percentage_dodds.png');
export_fig(image_filename,'-m6','-nocrop', '-transparent','-silent');
end
...
disp(sprintf('%s: Rho= %.3f, p=%.5f, CI95: %.3f %.3f',title_labels{term},r_rank,p_rank,bootstrap_results));
...
if (save_derivatives==1)
image_filename = strcat('derivatives/plot_sentiment_dodds_HDI_',lower(title_labels{term}),'.png');
export_fig(image_filename,'-m6','-nocrop', '-transparent','-silent');
end


%%%% Draw worldmaps of valence, arousal, dominance
...
%%%% Happiness difference across sexes
for term=1
...
%%%% Plot similarities between Warriner and Dodds
warriner=load('HEDO/sentiment_analysis_v20220329.mat');
[r_rank,p_rank]=corr(data_final(:,1),warriner.data_final(:,1),'type','spearman');
coor_ci = @(X,Y)corr(X,Y,'type','spearman');
bootstrap_results=bootci(1000,coor_ci,data_final(:,1),warriner.data_final(:,1));
disp(sprintf('Dodds vs Warriner: Rho= %.3f, p=%.5f, CI95: %.3f %.3f',r_rank,p_rank,bootstrap_results));


figure(); hold on;
scatter(data_final(:,1),warriner.data_final(:,1),40,'filled','MarkerFaceColor','k','MarkerFaceAlpha',.25,'MarkerEdgecolor','none');
if(p_rank<0.05)
l = lsline;
l.LineWidth = 1;
l.LineStyle='--';
...
axis square
if(draw_titles==1); title('Dodds'' ratings vs Warriner''s ratings'); end

xlabel('Dodds'' happiness scores','Fontsize',label_fontsize,'FontName','Arial','Fontweight','bold')
ylabel('Warriner''s valence ratings','Fontsize',label_fontsize,'FontName','Arial','Fontweight','bold')
xlim([min(data_final(:,1))-0.1, max(data_final(:,1))+0.1])
ylim([min(warriner.data_final(:,1))-0.1, max(warriner.data_final(:,1))+0.1])
...
if (save_derivatives==1)
image_filename = strcat('derivatives/plot_sentiment_dodds_vs_warriner.png');
export_fig(image_filename,'-m6','-nocrop', '-transparent','-silent');
end
