function SANe_Matlab2Gephi(GephiPrefix,AdjacencyMatrix,varargin)
...
addOptional(InputOptions,'NodeAttribute',[],@(x)assert(isnumeric(x) ... % NodeAttribute is an optional field. It has to be a numeric array or matrix,
    && sum(isnan(x(:)))==0,... % with no NaN
    'NodeAttribute is expected to be a n-by-p numeric matrix (or array with p=1) with no NaN'));
...
if ~isempty(AttributeLabel) % If node attributes are provided
    ...
    for N = 1:NumberOfNodes % For each node
        fprintf(fidNodeTable,'%g;%s;%s;%s\n',... % Write in node table
            ...
            strrep(strrep(strrep(mat2str(NodeAttribute(N,:),PrecisionOutput),' ',';'),'[',''),']','')); % Node Attributes
    end
else
