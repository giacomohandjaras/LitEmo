function varargout=notBoxPlot_nopoints(y,x,varargin)
...
params.addParameter('oneSided', 0, @(x) isnumeric(x) && isscalar(x));
...
oneSided   = params.Results.oneSided;
...
function [h,statsOut]=myPlotter(X,Y)
    ...
    if strcmp(style,'line') || strcmp(style,'sdline')
        for k=1:length(X)
            if(oneSided==0)
            h(k).sd=plot([X(k),X(k)],[mu(k)-SD(k),mu(k)+SD(k)],'-','color',[0.2,0.2,0.2],'linewidth',1);
            end
            if(oneSided==1)
            h(k).sd=plot([X(k),X(k)],[mu(k)-SD(k),mu(k)],'-','color',[0.2,0.2,0.2],'linewidth',1);
            end
            %if(oneSided==1)
            %h(k).sd=plot([X(k),X(k)],[mu(k),mu(k)+SD(k)],'-','color',[0.2,0.2,0.2],'linewidth',1);
            %end
            if(oneSided==2)
            h(k).sd=plot([X(k),X(k)],[mu(k)-SD(k),mu(k)],'-','color',[0.2,0.2,0.2],'linewidth',1);
            end
    ...
    function ptch=patchMaker(thisInterval,tColor)
        ...
        ptch=patch([X(k)-jitScale, X(k)+jitScale, X(k)+jitScale, X(k)-jitScale],[l,l,u,u], 0);        
