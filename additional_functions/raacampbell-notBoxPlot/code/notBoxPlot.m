function varargout=notBoxPlot(y,x,alpha,varargin)
...
function [h,statsOut]=myPlotter(X,Y)
    ...
    for k=1:length(X)
        ...
        if strcmp(style,'patch') 
            h(k).sdPtch=patchMaker(SD(k),[0.9,0.9,0.9],alpha);
        end

        %Build patch surfaces for SEM, the means, and optionally the medians
        if strcmp(style,'patch') || strcmp(style,'sdline')
            h(k).semPtch=patchMaker(SEM(k),[0.75,0.75,0.75],alpha);
            h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'-',...
                'linewidth',2, 'Color', [1,1,1]-alpha);
            if markMedian
                statsOut(k).median = med(k);
                h(k).med=plot([X(k)-jitScale,X(k)+jitScale],[med(k),med(k)],':',...
                    'linewidth',2, 'Color', [1,1,1]-alpha);
            end
        end
    ...
    function ptch=patchMaker(thisInterval,tColor,alpha)
        %This nested function builds a patch for the SD or SEM
        l=mu(k)-thisInterval;
        u=mu(k)+thisInterval;
        ptch=patch('XData',[X(k)-jitScale, X(k)+jitScale, X(k)+jitScale, X(k)-jitScale],...
                'YData',[l,l,u,u],'ZData', [-1,-1,-1,-1]);
        set(ptch,'edgecolor',tColor*0.8,'facecolor',tColor,'FaceAlpha',alpha,'EdgeAlpha',alpha)
