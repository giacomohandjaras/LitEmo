    ...
    if strcmpi(plotWhat,'singleplace')
         ...
         if faceplot
             ...
             for k = 1:length(nanz)-1
                 h(k) = patch(lon(nanz(k)+1:nanz(k+1)-1),lat(nanz(k)+1:nanz(k+1)-1),.5*[1 1 1],varargin{:},'EdgeColor','none'); 
             end
         ...
         if faceplot
             ...
             for k = 1:length(lat)
                 ...
                 for kk = 1:length(nanz)-1
                     h(kk) = patch(lonk(nanz(kk)+1:nanz(kk+1)-1),latk(nanz(kk)+1:nanz(kk+1)-1),.5*[1 1 1],varargin{:},'EdgeColor','none'); 
                 end
