function violins = violinplot(data, position, cats, varargin)
...
% This code is released under the terms of the BSD 3-clause license
    ...
    elseif ismatrix(data)
            ...
            thisData = data(:, position);
            violins = Violin(thisData, position, varargin{:});
        set(gca, 'xtick', position);
        if hascategories && length(cats) == size(data, 2)
            set(gca, 'xticklabels', cats{position});
        end
